...
2220-CHECK-TRANSACTION-INTEGRITY.
    READ TRANSACTION-HISTORY
        AT END
            CONTINUE
        NOT AT END
            MOVE CHANNEL-CODE TO WS-TRAN-CHANNEL-CODE *> Added for CHANNEL-CODE validation
...
2420-CHECK-TRANSACTION-FORMAT.
    READ TRANSACTION-HISTORY
        AT END
            CONTINUE
        NOT AT END
            MOVE CHANNEL-CODE TO WS-TRAN-CHANNEL-CODE *> Added for CHANNEL-CODE format check
            IF FUNCTION LENGTH(WS-TRAN-CHANNEL-CODE) NOT = 4
                MOVE 'INVALID CHANNEL-CODE LENGTH' TO WS-ERR-DESC
                WRITE ERROR-RECORD FROM WS-ERROR-LINE
            END-IF
    END-READ.
